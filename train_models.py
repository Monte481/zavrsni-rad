...
# Use the same ResNet-18 definition the detector expects, so checkpoints
# saved here can be loaded by detect.py without any key remapping.
...
class TrainConfig:
    ...
    # which model(s) to train this run
    mode: str = "both"            # "benign", "trojan", or "both"

    # attack settings
    attack: str = "badnets"       # "badnets" or "wanet"
    target_label: int = 0
    poison_rate: float = 0.1

    # badnets
    trigger_size: int = 4
    trigger_value: float = 1.0    # white patch in normalized [0,1] image space

    # wanet
    wanet_grid_rescale: float = 0.5
    wanet_noise_scale: float = 0.5
    wanet_identity_prob: float = 0.0  # can leave 0.0 for a simple implementation
...
def build_model(num_classes: int = 10) -> nn.Module:
    """
    CIFAR-style ResNet-18 imported from model.py. Using the same class as the
    detector means state-dict keys (stem.0/stem.1, shortcut, fc) match by
    construction.
    """
...
def apply_badnets_trigger(
    ...
    """
    Applies a square patch trigger to the bottom-right corner.
...
def create_wanet_noise_grid(
    ...
    """
    Create a small smooth noise grid and upsample it to image size.
    Returns shape [1, H, W, 2] suitable for grid_sample.
    """
    # random noise in [-1, 1]
    ins = torch.rand(1, 2, k, k, device=device) * 2 - 1
...
def create_identity_grid(image_size: int, device: torch.device) -> torch.Tensor:
    """
    Returns [1, H, W, 2] normalized sampling grid in [-1,1].
    """
...
# ============================================================
# Dataset wrapper for poisoning training samples
# ============================================================
...
class PoisonedCIFAR10(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # image comes from ToTensor() -> [C,H,W] in [0,1]
...
def evaluate_asr(
    ...
    """
    Attack Success Rate:
    among NON-target-label clean test samples, apply trigger and measure
    how often prediction becomes target_label.
    """
    model.eval()
    success = 0
    total = 0
...
def main():
    parser = argparse.ArgumentParser(description="Train benign and trojan CIFAR-10 models.")
