...
def _parse_args():
    p = argparse.ArgumentParser(description="Data-free Neural Cleanse detector.")
    ...
    p.add_argument("--data-source", choices=["clean", "synthetic"], default="synthetic",
                   help="Use clean data (baseline) or model-inverted synthetic data.")
    p.add_argument("--out-dir", default=None,
                   help="Directory for recovered triggers and summary. "
                        "Defaults to results_synthetic/<checkpoint-stem>/ for synthetic, "
                        "results/<checkpoint-stem>/clean/ for clean.")
    p.add_argument("--samples", type=int, default=config.VAL_SAMPLES,
                   help="(clean only) number of clean images to draw.")
    p.add_argument("--dataset", default="cifar10", choices=["cifar10", "mnist"],
                   help="(clean only) clean dataset for the NC baseline; must match training.")
    ...
    p.add_argument("--force", action="store_true",
                   help="Regenerate synthetic images even if cached.")
...
def _default_out_dir(checkpoint_path: str, data_source: str) -> str:
    ...
    stem = deep_inversion._checkpoint_key(checkpoint_path)
...
def main():
    ...
    print(f"[detect-df] device: {config.DEVICE}")
    print(f"[detect-df] data source: {args.data_source}")
    print(f"[detect-df] loading model: {args.model}")
    ...
    print(f"[detect-df] artifacts saved to: {out_dir}/")
